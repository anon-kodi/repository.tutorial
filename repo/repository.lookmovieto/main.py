...
def ListMovies(url, pg):
	...
	for i in range(len(ids[:-1])):
		...
		page = href
		page_html = sess.get(page, headers=headers, cookies=kukis, verify=False).text
		xbmc.log(f'[LookMovie2] page_html: {page_html}', xbmc.LOGDEBUG)
		
		href = href.replace('/movies/view/', '/movies/play/')
		href = href.replace('/shows/view/', '/shows/play/')

		#img = parseDOM(item,'img', ret="data-src")
		poster = parseDOM(page_html, 'p', attrs={'class': 'movie__poster lozad'}, ret='data-background-image')
		if not poster:
			style_attr = parseDOM(page_html, 'p', attrs={'class': 'movie__poster'}, ret='style')
			if style_attr:
				# Extract the image path from the style string
				match = re.search(r"url\((['\"]?)(/images/[^)]+)\1\)", style_attr[0])
				if match:
					poster.append(match.group(2))
		
		img = 'https://www.lookmovie2.to' + poster[0] if poster else ikona
		...
		if '/shows?' in url or '/shows/' in url:
			...
			plot_info = parseDOM(page_html, 'p', attrs={'class':'description '})
			plot = plot_info[0].strip() if plot_info else title
			mod = 'listlinks' 
...
def ListLinks(urlk, ima = None):
	...
	#xbmc.log(f'[LookMovie2] Access API response: {html}', xbmc.LOGDEBUG)
	#vid_source = list((html.get('streams', None)).values())[0]
	streams = html.get('streams')
	if not streams or not isinstance(streams,dict):
		xbmc.log(f'[LookMovie2] No video streams found. Response: {html}', xbmc.LOGERROR)
		return
	vid_source = None
	for quality in ['1080p', '720p', '480p', 'SD']:
		if streams.get(quality):
			vid_source = streams[quality]
			stream_quality = quality
			break

	if not vid_source:
		xbmc.log(f'[LookMovie2] No playable stream found. Streams: {streams}', xbmc.LOGERROR)
		return
	add_item(vid_source + '|' + urlk, 
		  '[B]' + title + '[/B]', 
		  ima, 
		  'playvid', 
		  fanart=FANART, 
		  folder=False, 
		  IsPlayable=True, 
		  infoLabels={'plot': plot,'year': year})
...
def router(paramstring):
	...
	if params:    
		...
		if mode == 'listmovies':
			...
			if 'rok' in ff:
				dd='year:'
				label = [str(x) for x in xrange(1913,2024)][::-1]#.insert(0,'all')
				label.insert(0,'all')
				value =  ['&y[]='+str(x) for x in xrange(1913,2024)][::-1]
				value.insert(0,'&y[]=')
			elif 'raty' in ff:
				dd= 'rating:'
